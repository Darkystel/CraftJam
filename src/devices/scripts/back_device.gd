...
func _ready():
	...
	additional_info = item_resource.additional_item_info
	$device/sprite.texture = item_resource.item_texture
...
func _physics_process(delta):
	...
	if Input.is_action_pressed("rmb_click") and battery > 0 and $cd.is_stopped():
		$device/sprite/light_origin/light.energy = lerp($device/sprite/light_origin/light.energy, additional_info['light-power'], 0.2)
		if affect_rc.is_colliding() and affect_rc.get_collider().is_in_group("enemy"):
			affect_rc.get_collider().on_light_cast(to_global($device/sprite/light_origin.position))
			if additional_info['damages_enemy']:
				affect_rc.get_collider().damage(30)
