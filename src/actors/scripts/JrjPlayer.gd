...
func _physics_process(delta):
	...
	if Input.is_action_just_pressed("jump") and not pick:
		if is_on_floor() :
			$character.play("jump")
	...
	if abs(velocity.x) > 0 and is_on_floor() and not pick:
		$character.play("walk")
	elif not is_on_floor() and velocity.y > 0 and not pick:
		$character.play("fall")
	elif is_on_floor() and not pick:
		$character.play("idle")
	if shake:
